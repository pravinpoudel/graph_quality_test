...
exports.greadability = void 0;

function _typeof(obj) { if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { _typeof = function _typeof(obj) { return typeof obj; }; } else { _typeof = function _typeof(obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; } return _typeof(obj); }

var greadability = function greadability(nodes, links, id) {
  var i,
      j,
      n = nodes.length,
      m,
  ...
  var graphStats = {
    crossing: 0,
    // Normalized link crossings
    crossingAngle: 0,
    // Normalized average dev from 70 deg
    angularResolutionMin: 0,
    // Normalized avg dev from ideal min angle
    angularResolutionDev: 0 // Normalized avg dev from each link

  };

  var getSumOfArray = function getSumOfArray(numArray) {
    var i = 0,
        n = numArray.length,
        sum = 0;
  ...
  var initialize = function initialize() {
    var i, j, link;
    var nodeById = {}; // Filter out self loops
    ...
    for (i = 0; i < m; ++i) {
      link = links[i];
      if (_typeof(link.source) !== "object") link.source = nodeById[link.source];
      if (_typeof(link.target) !== "object") link.target = nodeById[link.target];
    ...
    while (i < links.length) {
      if (links[i - 1].source.index === links[i].source.index && links[i - 1].target.index === links[i].target.index) {
        links.splice(i, 1);
      } else ++i;
    } // Update length, if a duplicate was deleted.


    m = links.length; // Calculate degree.
  ...
  function onSegment(pi, pj, pk) {
    return Math.min(pi[0], pj[0]) <= pk[0] && pk[0] <= Math.max(pi[0], pj[0]) && Math.min(pi[1], pj[1]) <= pk[1] && pk[1] <= Math.max(pi[1], pj[1]);
  }

  function linesCross(line1, line2) {
    var d1, d2, d3, d4; // CLRS 2nd ed. pg. 937
  ...
  function linksCross(link1, link2) {
    ...
    if (link1.index === link2.index || link1.source === link1.target || link2.source === link2.target) {
      return false;
    ...
    var line1 = [[link1.source.x, link1.source.y], [link1.target.x, link1.target.y]];
    var line2 = [[link2.source.x, link2.source.y], [link2.target.x, link2.target.y]];
  ...
  function linkCrossings() {
    var i,
        ...
        link2,
        line1,
        line2; // Sum the upper diagonal of the edge crossing matrix.

    for (i = 0; i < m; ++i) {
      for (j = i + 1; j < m; ++j) {
        link1 = links[i];
        link2 = links[j]; // Check if link i and link j intersect

        if (linksCross(link1, link2)) {
          line1 = [[link1.source.x, link1.source.y], [link1.target.x, link1.target.y]];
          line2 = [[link2.source.x, link2.source.y], [link2.target.x, link2.target.y]];
          ++c;
  ...
  function linesegmentsAngle(line1, line2) {
    ...
    var temp, len, angle1, angle2, sLine1, sLine2; // Re-orient so that line1[0] and line2[0] are the same.
    ...
    sLine1 = [[line1[0][0] - line1[0][0], line1[0][1] - line1[0][1]], [line1[1][0] - line1[0][0], line1[1][1] - line1[0][1]]]; // Normalize the line length.

    len = Math.hypot(sLine1[1][0], sLine1[1][1]);
    if (len === 0) return 0;
    sLine1[1][0] /= len;
    sLine1[1][1] /= len; // If y < 0, angle = acos(x), otherwise angle = 360 - acos(x)

    angle1 = Math.acos(sLine1[1][0]) * 180 / Math.PI;
    if (sLine1[1][1] < 0) angle1 = 360 - angle1; // Shift the line so that the first point is at (0,0).

    sLine2 = [[line2[0][0] - line2[0][0], line2[0][1] - line2[0][1]], [line2[1][0] - line2[0][0], line2[1][1] - line2[0][1]]]; // Normalize the line length.

    len = Math.hypot(sLine2[1][0], sLine2[1][1]);
    if (len === 0) return 0;
    sLine2[1][0] /= len;
    sLine2[1][1] /= len; // If y < 0, angle = acos(x), otherwise angle = 360 - acos(x)
  ...
  function acuteLinesAngle(line1, line2) {
    ...
    var slope2 = (line2[1][1] - line2[0][1]) / (line2[1][0] - line2[0][0]); // If these lines are two links incident on the same node, need
    ...
    if (slope1 === slope2) {
      ...
      if (!(onSegment(line1[0], line1[1], line2[0]) && onSegment(line1[0], line1[1], line2[1])) || !(onSegment(line2[0], line2[1], line1[0]) && onSegment(line2[0], line2[1], line1[1]))) return 180;else return 0;
    }
  ...
  function angularRes() {
    ...
    for (j = 0; j < n; ++j) {
      node = nodes[j];
      line0 = [[node.x, node.y], [node.x + 1, node.y]]; // Links that are incident to this node (already filtered out self loops)

      incident = degree[j];
      if (incident.length <= 1) continue;
      idealMinAngle = 360 / incident.length; // Sort edges by the angle they make from an imaginary vector
      ...
      incident.sort(function (a, b) {
        line1 = [[a.source.x, a.source.y], [a.target.x, a.target.y]];
        line2 = [[b.source.x, b.source.y], [b.target.x, b.target.y]];
      ...
      incidentLinkAngles = incident.map(function (l, i) {
        ...
        line1 = [[l.source.x, l.source.y], [l.target.x, l.target.y]];
        line2 = [[nextLink.source.x, nextLink.source.y], [nextLink.target.x, nextLink.target.y]];
    ...
    resMin = resMin / nonZeroDeg; // Divide by number of nodes with degree != 0
  ...
  initialize();
  cMax = m * (m - 1) / 2 - getSumOfArray(degree.map(function (d) {
...
//# sourceMappingURL=greadbility.dev.js.map
